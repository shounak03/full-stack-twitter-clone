...
const ProfilePage = () => {
	...
	if (!isLoading && !isRefetching && !user) return <p className='text-center text-lg mt-4'>User not found</p>;

	return (
		<div className='flex-[4_4_1] border border-gray-300 min-h-screen w-[50%]'>
			<div className='flex flex-col'>
				<div className='flex gap-10 px-4 py-2 items-center'>
					...
					<div className='flex flex-col'>
						<p className='font-bold text-lg'>{user?.data.fullname}</p>
						<span className='text-sm text-slate-500'>120 posts</span>
					</div>
				</div>
				{/* Cover Image */}
				<div className='relative group/cover'>
					...
					<div className='avatar absolute -bottom-16 left-4'>
						<div className='w-32 rounded-full relative group/avatar'>
							<img src={profileImg || user?.data.profileImg || "/avatar-placeholder.png"} alt="Profile" />
							{isMyProfile && (
				...
				<div className='flex flex-col gap-4 mt-14 px-4'>
					<div className='flex flex-col'>
						<span className='font-bold text-lg'>{user?.data.fullname}</span>
				...
				<div className='flex w-full border-b border-gray-700 mt-4'>
					...
					<div
						className='flex justify-center flex-1 p-3 text-slate-500 relative cursor-pointer'
			...
			<Posts feedType={feedType} username={username} userId={user?.data?._id} />
		</div>
	);
};
